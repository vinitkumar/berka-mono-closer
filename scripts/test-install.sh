...
mac_output=$(BERKA_DRY_RUN=1 BERKA_OS=Darwin "$installer" instrument)
assert_contains "$mac_output" "Dry run: would install Berka Mono Instrument into"
assert_contains "$mac_output" "BerkaMonoInstrument-Regular.ttf"
assert_contains "$mac_output" "BerkaMonoInstrument-BoldItalic.ttf"
...
printf '[test-install] installs selected local instrument fonts into temp directory\n'
font_dir="$tmp_root/fonts"
local_output="$tmp_root/local.out"
BERKA_OS=Linux BERKA_FONT_DIR="$font_dir" "$installer" --source-dir "$repo_dir" instrument >"$local_output"
assert_contains "$(cat "$local_output")" "Installed Berka Mono Instrument into $font_dir"
assert_contains "$(cat "$local_output")" "Verified 10 local TTF files for Berka Mono Instrument"
...
assert_contains "$(cat "$closer_output")" "Installed Berka Mono Closer into $font_dir"
...
[ -f "$font_dir/BerkaMonoCloser-BoldItalic.ttf" ] || fail "closer bold italic font was not installed"
